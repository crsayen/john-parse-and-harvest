...
import config.merlin as CONFIG
import time
import logging
...
def harvest_merlin():
    ...
    for report_folder_root in CONFIG.report_folder_roots:
        ...
        report_folders = [
            f for f in report_folders if os.path.isdir(report_folder_root + f)
        ]
        ...
        for report_folder in report_folders:
            ...
            for file in files:
                ...
                if already_in_db == False:
                    with open(
                        report_folder_root + "\\" + report_folder + "\\" + file,
                        encoding="latin-1",
                    ) as f:
                        s = f.read(600)
